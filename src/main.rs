...
fn buttons_panel(parent: &mut group::Flex) {
    frame::Frame::default();
    let w = frame::Frame::default().with_label("欢迎使用");
    ...
    {
        frame::Frame::default();
        let reg = create_button("注册");
        let login = create_button("登录");
...
fn middle_panel(parent: &mut group::Flex) {
    ...
    let mut frame = frame::Frame::default().with_label("Logo");
...
fn create_button(caption: &str) -> button::Button {
    button::Button::default().with_label(caption)
}
